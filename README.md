...
method is set.  
The current setting is for every second minute. If you want to for every minute then
change this line to the next:
```
image.service.cron.expression=0 * * * * *
```

**Do not forget to do a cleanup on your computer at the end!**  
The docker images and the volume for the db use significant disc space.